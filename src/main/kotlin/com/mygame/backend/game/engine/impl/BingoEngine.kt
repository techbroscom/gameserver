...
class BingoEngine : GameEngine {
    ...
    override fun onPlayerEvent(
        ...
        val calledNumbers =
            state.custom["calledNumbers"]?.jsonArray?.map { it.jsonPrimitive.int } ?: emptyList()
        ...
        updatedPlayers.forEach { (pid, pState) ->
            ...
            if (index != -1) {
                val marked = pState.custom["markedIndexes"]!!.jsonArray.map { it.jsonPrimitive.int }
                    .toMutableList()
                if (!marked.contains(index)) {
                    ...
                    // Check lines
                    val completed =
                        pState.custom["completedLines"]!!.jsonArray.map { it.jsonPrimitive.content }
                            .toMutableList()
        ...
        if (winners.isEmpty()) {
            ...
            if (winners.isNotEmpty()) {
                ...
                if (isDraw) {
                    ...
                    losers.forEach { pid ->
                        val score =
                            state.players[pid]?.custom?.get("lineCount")?.jsonPrimitive?.int ?: 0
                ...
                return GameResult(
                    ...
                    xpDeltas = xpDeltas,
                    summary = mapOf(
                        "totalNumbersCalled" to JsonPrimitive(
                            state.custom["calledNumbers"]?.jsonArray?.size ?: 0
                        )
                    )
                )
            }
